...
select last_name,department_id from emps where hir_date between('02-20-1998') and ('05-01
1998') order by hir_date asc; 
select last_name,department_id from emps where department_id in(20,50) order by last_name asc; 
select last_name as employee,salary as monthly_salary from emps where salary between 5000 and 
12000 and department_id in(20,50) order by last_name asc; 
...
select last_name,salary,comission from emps where comission is not null order by salary 
desc,comission desc; 
...
SELECT last_name, job_id, salary FROM emps WHERE job_id IN ('SA_REP', 'ST_CLERK') AND salary 
